...
对 Notion database **没有特殊结构要求，默认就能跑** —— 你不需要专门给这个插件新建一个 database，也不用改现有 database 的字段。

| 字段 | 必需性 | 行为 |
| --- | --- | --- |
| **Title 类型** | ✅ 必需 | 自动填文章标题。任何 Notion database 默认都有这个，通常叫 `Name` / `名称` / `标题`，不用你额外加。 |
| **URL 类型** | ⭕ 可选 | 如果有，自动填文章来源链接 |
| **Date 类型** | ⭕ 可选 | 如果有，自动填剪藏日期 |

> 识别是按**属性类型**做的，**不按名字** —— 字段叫 `Name` / `名称` / `标题` / `URL` / `Source` / `链接` / `剪藏日期` 都没关系，只要 type 对就行。同一类型有多个时会取**第一个**。

**其他自定义字段（Status / Tags / Multi-select / Relation / 公式 / 人员 / 你自己加的任何东西…）插件完全不会动**，新建页面里这些字段就是 Notion 自己的默认值，你可以后续手填，或者交给 Notion 自带的 Automation / 默认值规则自动填。

也就是说：**把这个插件接到你现有的任何「阅读 / 收藏 / 知识库」database 上都安全**，不会把别人精心设计的字段结构搞乱。
